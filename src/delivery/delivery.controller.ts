...
export class DeliveryController {
  ...
  async createDelivery(
    ...
    return res
      ...
      .json('Only Sender can create delivery');
  }

  @Get(':date')
  @UseGuards(JwtAuthGuard)
  async findDeliveryByDate(
    @Param() params: any,
    @Req() req,
    @Res() res,
  ): Promise<Delivery[]> {
    ...
    if (type == 'Courier') {
      ...
      const result = this.deliveryService.findAllCourierDeliveryByDate(
        date,
        type,
        username,
      );
      ...
      const result = this.deliveryService.findAllSenderDeliveryByDate(
        date,
        type,
      );
      return res.status(HttpStatus.OK).json(result);
  ...
  async assign(
    ...
    if (req.user.type != 'Sender') {
      return res
        .status(HttpStatus.FORBIDDEN)
        .json('Only sender can Assign deliveries');
    }
    const courier = await this.courierService.findByDate(
      params.couriername,
