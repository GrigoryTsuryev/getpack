...
export class CourierController {
    ...
    @UseGuards(JwtAuthGuard)
    async getRevenue(@Param() params: any, @Req() req, @Res()  res): Promise<Number> {
        if (params.dateTo && params.dateTo < params.dateFrom){
            res.status(HttpStatus.INTERNAL_SERVER_ERROR).json("Future date is more then today or from date")
        }
        return this.courierService.getRevenueByUsername(params.username, params.dateFrom, params.dateTo)
    }
