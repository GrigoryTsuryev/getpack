...
export class DeliveryService {
  constructor(
    ...
    private readonly deliveryModule: Model<DeliveryDocument>,
  ) {}
  ...
  async findAllCourierDeliveryByDate(
    date: string,
    type: string,
    username: string,
  ): Promise<Delivery[]> {
  ...
  async findAllSenderDeliveryByDate(
    date: string,
    type: string,
  ): Promise<Delivery[]> {
  ...
  async assignDelivery(courier: string, id: number): Promise<Delivery> {
    return this.deliveryModule.updateOne(
      { id },
      {
        courier,
      },
