...
export class CourierService {
    ...
    constructor(@InjectModel('couriers') private readonly courierModule: Model<CourierDocument>) { }
    ...
    async   create(courierDto: CreateCourierDTO): Promise<Courier> {
      ...
      async  assign(courierDto: CreateCourierDTO, date: string, addedRevenue: number, counter: number): Promise<Courier> {
        return this.courierModule.updateOne({ username: courierDto.name }, {
            ...courierDto,
            counter,
            date,
      ...
      async   getRevenueByUsername(username: string, dataFrom: string, dataTo: string): Promise<Number> {
        const documents =  await this.courierModule.find({username});
        return documents.filter(document => document.date < dataFrom)
